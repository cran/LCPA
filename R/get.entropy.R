...
#'   }
#'
#' @return A numeric value between 0 and 1 representing the relative entropy (Nylund-Gibson et al., 2018; Clark et al., 2013):
#'   \itemize{
...
#'
#' @export
get.entropy <- function(object){

  if (!("P.Z.Xn" %in% names(object))) {
    stop("Input object missing required element 'P.Z.Xn'")
  }

  if (is.null(object$params$P.Z)) {
    stop("Input object missing required element 'params$P.Z'")
  }

  P.Z.Xn <- object$P.Z.Xn
  N <- nrow(P.Z.Xn)
  L <- length(object$params$P.Z)
  entropy_i <- -rowSums(P.Z.Xn * log(P.Z.Xn + 1e-10))  # Add small constant to avoid log(0)
  entropy <- 1 - sum(entropy_i) / (N * log(L))
